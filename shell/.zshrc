...
# Source modular zsh fragments from the dotfiles repo. ~/.zshrc is a symlink
# into shell/.zshrc; siblings live in shell/zsh/*.zsh. Load order matters:
# widgets (10-, 20-) must be registered before bindkey references them (30-).
_zshrc_real="${$(readlink ~/.zshrc 2>/dev/null):-$HOME/.zshrc}"
_zsh_fragments_dir="${_zshrc_real:h}/zsh"
if [[ -d "$_zsh_fragments_dir" ]]; then
  for _f in "$_zsh_fragments_dir"/*.zsh(N); do
    source "$_f"
  done
fi
